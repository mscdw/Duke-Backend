...
# --- UPDATED PRIMARY ORCHESTRATOR FUNCTION ---
...
def process_all_faces_in_image(image_bytes: bytes, collection_id: str = DEFAULT_COLLECTION_ID) -> list:
  ...
  for face_detail in detected_face_details:
    # --- UPDATE 1: Filter on Detection Confidence ---
    # Only process faces that Rekognition is at least 90% confident are actual faces.
    ...
    result = {}
    try:
      # A. Search for the face, requiring a 90.0+ similarity score for a match.
      ...
      if search_response and search_response.get("FaceMatches"):
        ...
        # --- UPDATE 2: Use correct Similarity score for match confidence ---
        face_info = FaceInfo(
          ...
          Confidence=match_similarity  # Use the Similarity score for the match
        )
        result = {
          "status": "matched",
        ...
        if index_response and index_response.get("FaceRecords"):
          ...
          face_info = FaceInfo(
            ...
            Confidence=indexed_face_data.get("Confidence") # This confidence is correct for a new index
          )
          result = {
            "status": "indexed",
            "face_info": face_info.model_dump(),
